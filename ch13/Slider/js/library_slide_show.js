var slideshow = {
  ...
  changeSpeed: function (e) {
    ...
    console.log(this);
    var stime = prompt(
      `Thời gian slideshow hiện tại ${slideshow.speed} milliseconds\nMời bạn nhập thời gian mới (100->10000) milliseconds:`,
      2000
    );
    var regexTest = /^\d{3,5}$/;
    if (regexTest.test(stime) && stime >= 100 && stime <= 10000) {
      slideshow.speed = parseInt(stime);
      if (slideshow.play) {
        slideshow.stopSlideshow().startSlideshow();
      }
    } else {
      if (stime !== null) {
        alert('Thời gian phải từ 0.1s đến 10s');
      }
    }
  },
};
